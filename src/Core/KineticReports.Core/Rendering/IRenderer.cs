...
/// <summary>
/// Defines the contract for a renderer that converts a <see cref="ReportLayout"/> into
/// an output document (e.g. PDF, raster image, HTML stream).
/// </summary>
/// <remarks>
/// Renderers receive an immutable <see cref="ReportLayout"/> and must never perform
...
{
    /// <summary>
    /// Asynchronously renders the complete report layout to the specified
    /// <paramref name="output"/> stream.
    /// </summary>
    /// <param name="reportLayout">The immutable report layout to render.</param>
    ...
    Task RenderAsync(ReportLayout reportLayout, Stream output, CancellationToken cancellationToken = default);
}
