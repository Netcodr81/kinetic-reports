...
/// <summary>
/// Defines the contract for a renderer that converts a <see cref="LayoutTree"/> into
/// an output document (e.g. PDF, raster image, HTML stream).
/// </summary>
/// <remarks>
/// Renderers receive an immutable <see cref="LayoutTree"/> and must never perform
...
{
    /// <summary>
    /// Asynchronously renders the complete layout tree to the specified
    /// <paramref name="output"/> stream.
    /// </summary>
    /// <param name="tree">The immutable layout tree to render.</param>
    ...
    Task RenderAsync(LayoutTree tree, Stream output, CancellationToken cancellationToken = default);
}
